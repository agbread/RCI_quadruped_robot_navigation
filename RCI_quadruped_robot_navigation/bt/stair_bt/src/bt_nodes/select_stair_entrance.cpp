...
namespace stair_bt
{
  ...
  BT::NodeStatus SelectStairEntrance::tick()
  {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[SelectStairEntrance] 입력으로 받은 yaml_path: %s",
                yaml_path.c_str());
    ...
    if (yaml_path.empty() || yaml_path.find("$(") != std::string::npos)
    {
      try
      {
        ...
        RCLCPP_INFO(node_->get_logger(),
                    "[SelectStairEntrance] 기본 yaml_path로 설정됨: %s",
                    yaml_path.c_str());
      }
      catch (const std::exception &e)
      {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "[SelectStairEntrance] 최종 사용할 yaml_path: %s",
                yaml_path.c_str());
    ...
    if (sp)
    {
      ...
      {
        if (sp["flight"]["up"])
          flight_up = sp["flight"]["up"].as<double>();
        if (sp["flight"]["down"])
          flight_down = sp["flight"]["down"].as<double>();
      }
      if (sp["landing"])
      {
        if (sp["landing"]["up"])
          land_up = sp["landing"]["up"].as<double>();
        if (sp["landing"]["down"])
          land_down = sp["landing"]["down"].as<double>();
      }
    }
